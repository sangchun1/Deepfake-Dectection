...
def get_project_root() -> Path:
    ...
    repo root 반환.
    현재 파일 위치:
    src/deepfake_fusion/datasets/cifake_dataset.py
    """
...
class CIFAKEDataset(Dataset):
    ...
    기대하는 CSV 형식:
        filepath,label
        data/raw/cifake/train/REAL/xxx.jpg,0
        data/raw/cifake/train/FAKE/yyy.jpg,1

    또는:
        filepath,label
        train/REAL/xxx.jpg,0
        train/FAKE/yyy.jpg,1
    ...
    def __init__(
        ...
        self.root_dir = self._resolve_general_path(root_dir) if root_dir is not None else None
    ...
    def _resolve_image_path(self, filepath: str) -> Path:
        ...
        2) project root 기준 상대경로
           예: data/raw/cifake/train/REAL/xxx.jpg
        3) root_dir 기준 상대경로
           예: train/REAL/xxx.jpg
        """
        ...
        # validate_files=False일 때도 일관된 경로를 하나 반환하도록 처리
        if candidate_paths:
